...
\newcommand{\FN}[1]{\textcolor{orange}{[FN: #1]}}
...
precision over the candidate set, and $F_1$ their harmonic mean. We additionally report
wall-clock run time.
...
anisotropic partitioning to separate locally. The choice of index is therefore not orthogonal
to effectiveness on medical data, and the penalty falls on the largest collection, where
approximate retrieval is also the only affordable option (Table~\ref{tab:efficiency}).
...
(643{,}166 pairs), and short synonym-dense strings. It is also the only dataset where run time
becomes a bottleneck in its own right (Table~\ref{tab:efficiency}). We regard UMLS-scale
